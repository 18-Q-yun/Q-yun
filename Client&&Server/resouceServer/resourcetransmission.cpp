...
//---------------------------------------------Server------------------------------------------
void ResourceTransmission::serverUploadResource(std::shared_ptr<boost::asio::ip::tcp::socket> sock)
{
    string resourcePath;
...
void ResourceTransmission::serverDownloadResource(std::shared_ptr<boost::asio::ip::tcp::socket> sock)
{
    char str[MAX_DATA_LENGTH];
    try{
        sock->read_some(buffer(str, MAX_DATA_LENGTH));
