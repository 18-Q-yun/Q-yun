...
QList<Folder> FolderProxy::getUserFolder(QString request)
{
...
{
    QJsonParseError request_json_error;
    QString userName,currentDirName,folderName;
    int  currentLevel;
    //QTJSON所有的元素
    QJsonDocument request_doucment = QJsonDocument::fromJson(request.toUtf8(), &request_json_error);
    //检查json是否有错误
    if (request_json_error.error == QJsonParseError::NoError)
    {
        if (request_doucment.isObject())
        {
            ...
            {
                QJsonValue currentdirname_value = obj.take("currentDirName");
                if (currentdirname_value.isDouble())
                {
                    currentDirName = currentdirname_value.toVariant().toInt();
                }
            }
    ...
    FolderBroker folderBroker;
   folderBroker.deleteFolder(userName,folderName,currentDirName,currentLevel);
}

void FolderProxy::addFolder(QString request)
{
    if(request.left(10)=="firstApply"){
        FolderBroker folderBroker;
        folderBroker.addFolder(request.mid(11),QString::fromLocal8Bit("我的资源"),"/","1");
    }
      else
         {
    QJsonParseError request_json_error;
    ...
    FolderBroker folderBroker;
    folderBroker.addFolder(userName,folderName,currentDirName,currentLevel);
   }
}
